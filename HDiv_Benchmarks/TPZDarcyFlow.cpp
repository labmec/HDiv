...
using namespace std;
STATE TPZDarcyFlow::gAlfa = 0.5;

TPZDarcyFlow::TPZDarcyFlow(int nummat, int dim) : TPZRegisterClassId(&TPZDarcyFlow::ClassId),
...
TPZDarcyFlow::TPZDarcyFlow(const TPZDarcyFlow &copy):TPZRegisterClassId(&TPZDarcyFlow::ClassId),
...
void TPZDarcyFlow::SetParameters(STATE diff, REAL conv, TPZVec<REAL> &convdir) {
    fK = diff;
    fC = conv;
    int d;
...
void TPZDarcyFlow::GetParameters(STATE &diff, REAL &conv, TPZVec<REAL> &convdir) {
    diff = fK;
    conv = fC;
    int d;
...
int TPZDarcyFlow::NStateVariables() {
    return 1;
}
    ...
    if(fC != 0.0) {
        ...
        switch(fDim) {
            ...
            case 3:
                ...
                break;
            default:
                PZError << "TPZDarcyFlow::Contribute dimension error " << fDim << endl;
        }
    }
...
void TPZDarcyFlow::ContributeHDiv(TPZMaterialData &data,REAL weight,TPZFMatrix<STATE> &ek,TPZFMatrix<STATE> &ef)
{
    /** monta a matriz
     |A B^T |  = |0 |
     |B 0   |    |f |
...
//
void TPZDarcyFlow::LocalNeumanContribute(TPZMaterialData &data,REAL weight,TPZFMatrix<STATE> &ek,TPZFMatrix<STATE> &ef)
{
    /** monta a matriz
     |Sk Ck^T |  = |bk|
     |Ck  0   |    |uk|
...
void TPZDarcyFlow::ContributeBCHDiv(TPZMaterialData &data,REAL weight,
...
void TPZDarcyFlow::ContributeBC(TPZMaterialData &data,REAL weight,
                                   TPZFMatrix<STATE> &ek,TPZFMatrix<STATE> &ef,TPZBndCond &bc) {
...
void TPZDarcyFlow::Solution(TPZVec<STATE> &Sol,TPZFMatrix<STATE> &DSol,TPZFMatrix<REAL> &axes,int var,TPZVec<STATE> &Solout){
...
void TPZDarcyFlow::Flux(TPZVec<REAL> &/*x*/, TPZVec<STATE> &/*Sol*/, TPZFMatrix<STATE> &/*DSol*/, TPZFMatrix<REAL> &/*axes*/, TPZVec<STATE> &/*flux*/) {
    //Flux(TPZVec<REAL> &x, TPZVec<REAL> &Sol, TPZFMatrix<REAL> &DSol, TPZFMatrix<REAL> &axes, TPZVec<REAL> &flux)
}
void TPZDarcyFlow::ErrorsHdiv(TPZMaterialData &data,TPZVec<STATE> &u_exact,TPZFMatrix<STATE> &du_exact,TPZVec<REAL> &values){
...
void TPZDarcyFlow::Errors(TPZVec<REAL> &x,TPZVec<STATE> &u,
...
void TPZDarcyFlow::BCInterfaceJump(TPZVec<REAL> &x, TPZSolVec &leftu,TPZBndCond &bc,TPZSolVec & jump){
...
#ifdef _AUTODIFF
void TPZDarcyFlow::ContributeEnergy(TPZVec<REAL> &x,
...
void TPZDarcyFlow::ContributeBCEnergy(TPZVec<REAL> & x,TPZVec<FADFADREAL> & sol, FADFADREAL &U, REAL weight, TPZBndCond &bc) {
    REAL vartocast = 0.;
#ifdef STATE_COMPLEX
...
void TPZDarcyFlow::ContributeInterface(TPZMaterialData &data, TPZMaterialData &dataleft, TPZMaterialData &dataright,
                                          REAL weight,
...
void TPZDarcyFlow::ContributeBCInterface(TPZMaterialData &data, TPZMaterialData &dataleft,
                                            REAL weight,
                                            TPZFMatrix<STATE> &ek,TPZFMatrix<STATE> &ef,TPZBndCond &bc){
...
void TPZDarcyFlow::InterfaceErrors(TPZVec<REAL> &/*x*/,
...
REAL TPZDarcyFlow::ComputeSquareResidual(TPZVec<REAL>& X, TPZVec<STATE> &sol, TPZFMatrix<STATE> &dsol){
