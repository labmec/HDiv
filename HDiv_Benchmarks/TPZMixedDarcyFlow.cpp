...
TPZMixedDarcyFlow::TPZMixedDarcyFlow(int matid, int dim): TPZRegisterClassId(&TPZMixedDarcyFlow::ClassId), TPZDarcyFlow(matid,dim) {
    if (dim < 1) {
        DebugStop();
    }
    fvisc = 1.;
...
int TPZMixedDarcyFlow::ClassId() const{
    return Hash("TPZMixedDarcyFlow") ^ TPZMixedDarcyFlow::ClassId() << 1;
}
